...
#if defined(NDEBUG)
# error "Exmy cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8cb6e9adb9adeca5e6f62e9abb4f434099b5856fe89d2afd73ccab16f5dbaa37");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Exmy: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Exmy Signed Message:\n";
...
{
    // Exmy: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 5000000 blocks, which will occur approximately every 2.5 years
    nSubsidy >>= (nHeight / 5000000); // Exmy: 840k blocks in ~2.5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 10; // Exmy: 30 Seconds.
static const int64 nTargetSpacing = 1 * 10; // Exmy: 10 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Exmy: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Exmy: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8cb6e9adb9adeca5e6f62e9abb4f434099b5856fe89d2afd73ccab16f5dbaa37");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("033284710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1547127015;
        ...
        assert(block.hashMerkleRoot == uint256("0x5e3d5ab2a027ef6bce7653300ccf7445d32a5a8ab0bb3a7259d557c5178da7d7"));
...
unsigned char pchMessageStart[4] = { 0xc4, 0xd4, 0xd4, 0xb1 }; // Exmy: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ExmyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ExmyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ExmyMiner(CWallet *pwallet)
{
    printf("ExmyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("exmy-miner");
    ...
    try { loop {
        ...
        printf("Running ExmyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ExmyMiner, pwallet));
}
